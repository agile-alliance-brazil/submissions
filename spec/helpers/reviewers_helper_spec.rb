...
describe ReviewersHelper, type: :helper do
  it 'replies doesnot_review for track without preferences' do
    level = helper.review_level([], FactoryBot.build(:track))
    expect(level).to eq('reviewer.doesnot_review')
  end

  it "replies doesnot_review for track with preferences that don't match" do
  ...
  it 'replies preference level for track with preferences that match' do
  ...
  it 'builds hash with reviewers when reviewer is anonymous' do
  ...
  it 'builds hash with reviewers when reviewer is not anonymous' do
    early_review = FactoryBot.create(:early_review)
  ...
  it 'builds hash with reviewers when there are anonymous and not anonymous reviewers' do
  ...
  context 'reviewer summary review row' do
    ...
    it 'has 4 0s and a blank spot before reviews are happening' do
    ...
    it 'partitions reviews by recommendations' do
    ...
    it 'includes feedback if past author notification' do
      @conference.save
      reviews = @recommendations.map do |r|
  ...
  context 'review feedback score' do
    ...
    it 'counts a negative feedback as 10' do
