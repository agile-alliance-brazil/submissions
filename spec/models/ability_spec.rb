...
describe Ability, type: :model do
  ...
  shared_examples_for 'all users' do
    ...
    it 'can read session related pages' do
  ...
  describe '- all users (guests)' do
    it_behaves_like 'all users'

    it 'cannot manage reviewer' do
      expect(ability).not_to be_able_to(:manage, Reviewer)
    end

    it 'cannot read organizers' do
      expect(ability).not_to be_able_to(:read, Organizer)
    end

    it 'cannot read reviews' do
      expect(ability).not_to be_able_to(:read, Review)
      expect(ability).not_to be_able_to(:read, FinalReview)
      expect(ability).not_to be_able_to(:read, EarlyReview)
    end

    it 'cannot read reviews listing' do
      expect(ability).not_to be_able_to(:read, 'reviews_listing')
    end

    it 'cannot read sessions to organize' do
      expect(ability).not_to be_able_to(:read, 'organizer_sessions')
    end

    it 'cannot read organizer reports' do
      expect(ability).not_to be_able_to(:read, 'organizer_reports')
    end

    it 'cannot read accepted sessions reports' do
      expect(ability).not_to be_able_to(:read, 'accepted_sessions')
    end

    it 'cannot read sessions to review' do
      expect(ability).not_to be_able_to(:read, 'reviewer_sessions')
    end

    describe 'if reviewer' do
      ...
      it 'can accept reviewer invitation if invited' do
        expect(ability).not_to be_able_to(:manage, 'accept_reviewers')
      ...
      it 'can reject reviewer invitation if invited' do
        expect(ability).not_to be_able_to(:manage, 'reject_reviewers')
  ...
  describe '- author' do
    ...
    describe 'index early reviews of' do
      ...
      it 'other peoples sessions is forbidden' do
        expect(ability).not_to be_able_to(:index, EarlyReview)
        expect(ability).not_to be_able_to(:index, EarlyReview, session)
    ...
    describe 'index final reviews of' do
      ...
      it 'his sessions if review has been published' do
        ...
        expect(ability).not_to be_able_to(:index, FinalReview, session)
      end
    ...
    describe 'with regards to create sessions:' do
      ...
      context 'when in a conference with submission limits' do
        before do
        ...
        it 'cannot in submissions phase and already at the limit of submissions' do
          ...
          expect(ability).not_to be_able_to(:create, Session)
        end
      end
      ...
      it 'overides admin privileges to check for deadlines' do
        ...
        expect(ability).not_to be_able_to(:create, Session)
        expect(ability).not_to be_able_to(:new, Session)
      end
    end
    ...
    describe 'can cancel session if:' do
    ...
    describe 'can confirm session if:' do
      ...
      it '- user is first author' do
        expect(ability).not_to be_able_to(:manage, 'confirm_sessions')
    ...
    describe 'can withdraw session if:' do
      ...
      it '- user is first author' do
        expect(ability).not_to be_able_to(:manage, 'withdraw_sessions')
    ...
    describe 'create review feedback' do
      let(:session) do
        FactoryBot.build(:session,
                         author: user,
      ...
      it 'any review if session does not have a review decision' do
        session.review_decision = nil
        expect(ability).not_to be_able_to(:create, ReviewFeedback)
      end
  ...
  describe '- organizer' do
    ...
    it 'can read reviews listing' do
      ...
      expect(ability).not_to be_able_to(:reviewer, 'reviews_listing')
    end
    ...
    describe 'organizer index reviews of' do
      ...
      it 'session on organizers track is allowed' do
        ...
        expect(ability).not_to be_able_to(:organizer, FinalReview)
        expect(ability).not_to be_able_to(:organizer, EarlyReview)
    ...
    describe 'can create review decision if:' do
      let(:track) { FactoryBot.create(:track, conference: conference) }
      let(:session) { FactoryBot.build(:session, conference: conference, track: track) }
      ...
      it '- session on organizers track' do
        expect(ability).not_to be_able_to(:create, ReviewDecision, session)
        expect(ability).not_to be_able_to(:create, ReviewDecision)
    ...
    describe 'can edit review decision session' do
      let(:track) { FactoryBot.create(:track, conference: conference) }
      let(:session) { FactoryBot.build(:session, conference: conference, track: track) }
      ...
      it 'if session on organizers track' do
        expect(ability).not_to be_able_to(:update, ReviewDecision, session)
        expect(ability).not_to be_able_to(:update, ReviewDecision)
  ...
  describe '- reviewer' do
    ...
    it 'cannot read other reviewers' do
      expect(ability).not_to be_able_to(:read, Reviewer)
    end
    ...
    it 'cannot index all reviews of any session' do
      expect(ability).not_to be_able_to(:index, Review)
      expect(ability).not_to be_able_to(:index, FinalReview)
      expect(ability).not_to be_able_to(:index, EarlyReview)
    end

    it 'cannot index all organizer reviews of any session' do
      expect(ability).not_to be_able_to(:organizer, Review)
      expect(ability).not_to be_able_to(:organizer, FinalReview)
      expect(ability).not_to be_able_to(:organizer, EarlyReview)
    end
    ...
    it 'can show own early reviews' do
      ...
      expect(ability).not_to be_able_to(:show, review)
    ...
    describe 'can create a new final review if:' do
      ...
      it 'has a session available to add the final review to' do
        expect(ability).not_to be_able_to(:create, FinalReview)
        expect(ability).not_to be_able_to(:create, FinalReview, nil)
      ...
      it 'after final review deadline cant review' do
        ...
        expect(ability).not_to be_able_to(:create, FinalReview)
        expect(ability).not_to be_able_to(:create, FinalReview, nil)
        expect(ability).not_to be_able_to(:create, FinalReview, session)
      end
    ...
    describe 'can create a new early review if:' do
      let(:session) { FactoryBot.build(:session) }
      ...
      it 'has a session available to add the early review to' do
        expect(ability).not_to be_able_to(:create, EarlyReview)
        expect(ability).not_to be_able_to(:create, EarlyReview, nil)
      ...
      it 'after early review deadline cant review' do
        ...
        expect(ability).not_to be_able_to(:create, EarlyReview)
        expect(ability).not_to be_able_to(:create, EarlyReview, nil)
        expect(ability).not_to be_able_to(:create, EarlyReview, session)
      end
  ...
  describe '- voter' do
    ...
    describe 'can create a new vote if:' do
      before do
      ...
      it 'within limit for conference with session' do
        ...
        expect(ability).not_to be_able_to(:create, Vote, session)
      end

      it 'within limit for conference without session' do
        Vote.expects(:within_limit?).with(user, conference).twice.returns(true, false)
        expect(ability).to be_able_to(:create, Vote)
        expect(ability).not_to be_able_to(:create, Vote)
      end
      ...
      it 'after voting deadline cant vote' do
        ...
        ability = Ability.new(user, conference, session)
        expect(ability).not_to be_able_to(:create, Vote)
        expect(ability).not_to be_able_to(:create, Vote, nil)
        expect(ability).not_to be_able_to(:create, Vote, session)
      end
    end

    describe 'can destroy votes if:' do
