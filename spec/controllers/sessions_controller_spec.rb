...
describe SessionsController, type: :controller do
  ...
  it "show action should display flash news if session from previous conference" do
    ...
    message = I18n.t('flash.news.session_different_conference',
      ...
      current_conference_name: Conference.current.name,
  ...
  it "create action should redirect when model is valid" do
    ...
    post :create
    expect(response).to redirect_to(session_url(Conference.current, assigns(:session)))
  end
  ...
  it "edit action should only assign tracks for current conference" do
    get :edit, id: Session.first
    expect((assigns(:tracks) - Conference.current.tracks)).to be_empty
  end

  it "edit action should only assign audience levels for current conference" do
    get :edit, id: Session.first
    expect((assigns(:audience_levels) - Conference.current.audience_levels)).to be_empty
  end

  it "edit action should only assign session types for current conference" do
    get :edit, id: Session.first
    expect((assigns(:session_types) - Conference.current.session_types)).to be_empty
  end
  ...
  it "update action should redirect when model is valid" do
    ...
    expect(response).to redirect_to(session_path(Conference.current, assigns(:session)))
  end

  it "cancel action should cancel and redirect to organizer sessions" do
    delete :cancel, id: Session.first
    expect(response).to redirect_to(organizer_sessions_path(Conference.current))
  end
