...
describe ReviewsController, type: :controller do
  before(:each) do
    ...
    @conference = @early_review.session.conference
  ...
  it "new action should set reviewer" do
    get :new, session_id: Session.first
    expect(assigns(:review).reviewer).to eq(@final_review.reviewer)
  end
  ...
  it "create action should redirect when final review is valid" do
    ...
    expect(response).to redirect_to(session_review_path(@conference, assigns(:session), assigns(:review)))
  end
