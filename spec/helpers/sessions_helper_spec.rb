...
describe SessionsHelper do
  ...
  describe "#all_durations_for" do
    ...
    context "single session types" do
      subject { helper.all_durations_for([Factory.build(:session_type, :valid_durations => [10, 20])]) }
      it { should == [10, 20]}
    end
    ...
    context "multiple session types" do
      it "should merge durations" do
        durations = helper.all_durations_for([
          Factory.build(:session_type, :valid_durations => [10, 20]),
          Factory.build(:session_type, :valid_durations => [30, 40]),
        ])
      ...
      it "should remove duplicates" do
        durations = helper.all_durations_for([
          Factory.build(:session_type, :valid_durations => [10, 20]),
          Factory.build(:session_type, :valid_durations => [20, 30]),
        ])
      ...
      it "should sort durations" do
        durations = helper.all_durations_for([
          Factory.build(:session_type, :valid_durations => [20, 40]),
          Factory.build(:session_type, :valid_durations => [30, 10]),
        ])
        durations.should == [10, 20, 30, 40]
      end
    end
  ...
  describe "#durations_to_hide" do
    it "should return durations to hide as strings" do
      session_type_1 = Factory.create(:session_type, :valid_durations => [20, 40])
      session_type_2 = Factory.create(:session_type, :valid_durations => [10, 20])
    ...
    it "should hide default option when session type only accepts a single duration" do
      session_type_1 = Factory.create(:session_type, :valid_durations => [40])
      session_type_2 = Factory.create(:session_type, :valid_durations => [10, 20])
      durations_to_hide = helper.durations_to_hide([session_type_1, session_type_2])
  ...
  describe "#duration_mins_hint" do
    it "should generate hint from session types in portuguese" do
      I18n.with_locale('pt') do
        hint = helper.duration_mins_hint([
          Factory.build(:session_type, :title => "session_types.talk.title", :valid_durations => [20, 40]),
          Factory.build(:session_type, :title => "session_types.experience_report.title", :valid_durations => [10]),
          Factory.build(:session_type, :title => "session_types.hands_on.title", :valid_durations => [30, 20])
        ])
    ...
    it "should generate hint from session types in english" do
      I18n.with_locale('en') do
        hint = helper.duration_mins_hint([
          Factory.build(:session_type, :title => "session_types.workshop.title", :valid_durations => [20]),
          Factory.build(:session_type, :title => "session_types.hands_on.title", :valid_durations => [40])
        ])
