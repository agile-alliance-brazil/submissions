...
RSpec.describe SessionsHelper, type: :helper do
    ...
    context 'multiple session types' do
      it 'merges durations' do
        durations = helper.all_durations_for([
                                               FactoryBot.build(:session_type, valid_durations: [10, 20]),
                                               FactoryBot.build(:session_type, valid_durations: [30, 40])
                                             ])
      ...
      it 'removes duplicates' do
        durations = helper.all_durations_for([
                                               FactoryBot.build(:session_type, valid_durations: [10, 20]),
                                               FactoryBot.build(:session_type, valid_durations: [20, 30])
                                             ])
      ...
      it 'sorts durations' do
        durations = helper.all_durations_for([
                                               FactoryBot.build(:session_type, valid_durations: [20, 40]),
                                               FactoryBot.build(:session_type, valid_durations: [30, 10])
                                             ])
        expect(durations).to eq([10, 20, 30, 40])
      end
    end
  ...
  describe '#durations_to_hide' do
    it 'returns durations to hide as strings' do
      session_type1 = FactoryBot.create(:session_type, valid_durations: [20, 40])
      session_type2 = FactoryBot.create(:session_type, valid_durations: [10, 20])
    ...
    it 'hides default option when session type only accepts a single duration' do
      session_type1 = FactoryBot.create(:session_type, valid_durations: [40])
      session_type2 = FactoryBot.create(:session_type, valid_durations: [10, 20])
      durations_to_hide = helper.durations_to_hide([session_type1, session_type2])
  ...
  context 'type titles with language set' do
    ...
    describe '#duration_mins_hint' do
      let(:relato) { FactoryBot.build :translated_content, language: 'pt-BR', title: 'Relato de Experiência' }
      let(:na_massa) { FactoryBot.build :translated_content, language: 'pt-BR', title: 'Mão na massa' }
      let(:workshop) { FactoryBot.build :translated_content, language: 'en', title: 'Workshop' }
      let(:hands_on) { FactoryBot.build :translated_content, language: 'en', title: 'Hands on' }

      it 'generates hint from session types in portuguese' do
        I18n.with_locale('pt-BR') do
          hint = helper.duration_mins_hint([
                                             FactoryBot.build(:session_type, conference: conference, translated_contents: [palestra], valid_durations: [20, 40]),
                                             FactoryBot.build(:session_type, conference: conference, translated_contents: [relato], valid_durations: [10]),
                                             FactoryBot.build(:session_type, conference: conference, translated_contents: [na_massa], valid_durations: [30, 20])
                                           ])
      ...
      it 'generates hint from session types in english' do
        I18n.with_locale('en') do
          hint = helper.duration_mins_hint([
                                             FactoryBot.build(:session_type, conference: conference, translated_contents: [workshop], valid_durations: [20]),
                                             FactoryBot.build(:session_type, conference: conference, translated_contents: [hands_on], valid_durations: [40])
                                           ])
