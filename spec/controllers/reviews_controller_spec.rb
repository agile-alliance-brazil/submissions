...
describe ReviewsController, type: :controller do
  ...
  context 'disabled authorization' do
    ...
    context 'create action should create a third reject' do
      let(:reject_recommendation) { Recommendation.create(name: 'weak_reject') }
      let(:approve_recommendation) { Recommendation.create(name: 'weak_approve') }
      ...
      it 'not when this is the third review' do
      ...
      it 'when this is a rejection and there is only one rejection' do
        session.final_reviews.new(recommendation: reject_recommendation).save(validate: false)
        ...
        expect(session).to have(3).final_reviews
        expect(session.final_reviews.map(&:recommendation)).to all(be_weak_reject)
      end

      it 'not when this is an approve review and there is one rejection' do
        session.final_reviews.new(recommendation: reject_recommendation).save(validate: false)
